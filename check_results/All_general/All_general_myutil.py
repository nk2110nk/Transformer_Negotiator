...
for i, agent0 in enumerate(AGENT_LIST):
    for j in range(i, len(AGENT_LIST)):
        ...
        for issue in ISSUE_NAMES:
            file_path = f"{path}/{pair}/{issue}/det=False_noise=False/{issue}-{agent0}-{agent1}-dF-nF.tsv"

            if not os.path.exists(file_path):
                print("missing:", file_path)
                continue

            result = pd.read_csv(file_path, delimiter='\t')
            results_all.loc[issue, pair] = result['my_util'].mean()
...
print(results_all)
results_all.to_csv("summary_util.csv")
